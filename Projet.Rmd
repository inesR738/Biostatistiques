...
sum(Intoxications$Boeuf %>% as.integer() %>% -1 & 
      Intoxications$Oeuf %>% as.integer() %>% -1 & 
      Intoxications$Eau %>% as.integer() %>% -1, na.rm=T)
```
...
sum(Intoxications$Nausee %>% as.integer() %>% -1 & 
      Intoxications$Vomi %>% as.integer() %>% -1 &
      Intoxications$Douleur %>% as.integer() %>% -1 &
      Intoxications$Diarrhee %>% as.integer() %>% -1, na.rm=T)
```
...
Maintenant que nous savons que quasiment la moitié de la population est tombé malade après cette manifestation, nous allons tenter de savoir quels sont les facteurs qui ont ammené à cette intoxication. Nous allons le faire avec deux modèle de regression (logistique et polytomique ordonné)

## Regression logistique

Comme nous avons pu le voir dans la partie précédente, nous avons créé une variable $Malade$ comme variable qualitative codé par 0 ou 1, d'où l'utilisation d'une regression logistique.
...
Nous allons analyser la significativité des coefficients du modèle final que nous avons obtenu pour cette regression:
...
```{r, echo=FALSE}
Intoxications %>% pull(var = Symptomes) %>% summary()
```
...
Nous obtenons le modèle suivant: $$ Symptomes \sim Eclair + Eau + Sexe + Age + Eclair:Eau + Eclair:Sexe + Sexe:Age + Eclair:Age + Eclair:Sexe:Age$$ Ce modèle est plus complexe que le modèle de régression logistique de la partie précedente, d'où son AIC plus élevé.
...
```{r}
f <- polr(Symptomes ~ Eclair + Sexe + Age + Eclair:Sexe + Eclair:Age + Eclair:Sexe:Age, data = Intoxications)

summary(f)
```

Le modèle gardé est: $$Symptomes \sim Eclair + Sexe + Age + Eclair:Sexe + Eclair:Age + Eclair:Sexe:Age$$ Il est très semblable au modèle de regression logistique obtenu dans la partie précédente.
